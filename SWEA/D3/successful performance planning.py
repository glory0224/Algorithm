...
for tc in range(1, T+1):
    ...
    for i in range(1, len(claps)):
        if cnt < i: # 현재까지 박수를 치고 있는 사람들의 수가 i보다 작은 경우  
            answer += i - cnt # i - cnt 만큼 인원 수 고용
            cnt = i # i명을 고용했기 때문에 cnt를 i로 갱신
        
        cnt += int(claps[i]) # i보다 작지 않은 경우 박수를 치는 경우이기 때문에 cnt를 claps[i] 만큼 증가한다.
