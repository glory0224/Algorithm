...
# 문제의 핵심은 각 행과 열의 길이를 비교하면서 열의 길이가 행의 길이를 넘어서는 경우를 제외하고 문자열에 대입시키는 방법이다.
...
for tc in range(1, T+1):
    ...
    # 각 단어의 길이를 max_len으로 맞추기
    for i in range(5):
        # 현재 행의 길이
        if len(matrix[i]) < max_len:
            matrix[i] += ' ' * (max_len - len(matrix[i])) # 현재 행의 길이가 최대 길이보다 작으면 그 행에 공백으로 리스트를 채운다. 
