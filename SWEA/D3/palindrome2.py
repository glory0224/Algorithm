...
# 회문의 개수가 아닌 최대 길이 문제 (100 * 100 크기 - 브루트 포스 사용 가능)
...
def find_palindrome(board):
    ...
    max_len = 1 # 최소 회문 기준 1

    # 행 돌면서 최대 회문 길이 확인

    for i in range(n):
        for j in range(n - max_len): # 최대 길이만큼 빼줘서 최대 길이보다 큰건 이미 최대 길이로 적용된 경우라 돌지 않아도 된다. 
            for k in range(max_len, n-j+1): # 회문의 길이를 최대 길이부터
                is_palindrome = True
                for l in range(k // 2): # 회문 길이를 // 연산으로 절반 나누면 가운데 문자를 기준으로 비교 가능
                    if board[i][j+l] != board[i][j+k-l-1]:
                        is_palindrome = False
                        break # 회문이 아니기 때문에 종료
                
                if is_palindrome: # 회문일 경우
                    max_len = k # 최대 길이 갱신
    ...
    for i in range(n):
        for j in range(n - max_len):
            for k in range(max_len, n-j+1):
                is_palindrome = True
                for l in range(k // 2):
                    if board[j+l][i] != board[j+k-l-1][i]:
                        is_palindrome = False
                        break
