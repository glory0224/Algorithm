...
# 회문인지 아닌지와 각 문자열과 문자열 역순에 '*'가 포함되어 있는지 확인하는 문제, 세상 모든 팰린드롬 1 문제와 유사

def is_possible_palindrome(word):
    ...
    for i in range(len(word)):
        ...
        # 원래 문자열과 역순 문자열이 회문이 아니고 문자열, 역순 중 하나라도 '*'가 포함되는 경우
        if word[i] == '*' or reverse_word[i] == '*':
            answer = 'Exist'
            break
        # 원래 문자열과 역순 문자열이 회문이 아니고 포함되지 않는 경우 
        else:
            break
    
    else:
        answer = 'Exist' # for문이 종료되지 않고 왔다는건 word[i] == reverse_word[i] 경우이기 때문에 회문 처리
...
for tc in range(1, T+1):
    ...
    answer = is_possible_palindrome(word)
