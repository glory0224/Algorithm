...
# 이렇게 원래의 문자열을 까먹는 일이 많은 동욱이는 원래의 문자열을 복원해 내기 위한 정보를 만들어 놓았고, 그 정보는 인접한 두 문자를 끊어 봤을 때 각각의 쌍이 몇 번씩 등장하는지를 적어 놓은 것이다.

# 단, 인접한 두 문자를 끊어 볼 때 두칸씩 이동하지 않고 한칸만 이동하여 본다.

# 예를 들어 “01000110”는 인접한 두 문자를 끊어 봤을 때, “01”, “10”, “00”, “00”, “01”, “11”, “10”의 일곱 가지의 문자열이 있고 동욱이는 이를 “00” 두 개, “01” 두 개, “10” 두 개, “11” 한 개로 적어 놓은 것이다.
...
for tc in range(1, T+1):
    ...
    elif abs(B - C) > 1: # 01 과 10 의 개수 차이가 2이상
        result = "impossible"
    
    else:
        if B == 0 and C == 0:
            # 01과 10이 없는데 00이 개수가 있다면
            if A != 0:
                result = '0' * (A+1) # '00' 넣기
            # 01과 10, 00 이 없고 '11'만 있다면
            else:
                result = '1' * (D+1) # '11' 넣기
